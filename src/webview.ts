...
export function registerWebviewViewProvider(context: vscode.ExtensionContext) {
  vscode.window.registerWebviewViewProvider("craView", {
    resolveWebviewView: async (webviewView) => {
      ...
      const resetIconPath = webviewView.webview.asWebviewUri(
        vscode.Uri.joinPath(context.extensionUri, "media", "reset-icon.png")
      );
      const sendIconPath = webviewView.webview.asWebviewUri(
        vscode.Uri.joinPath(context.extensionUri, "media", "send-icon.png")
      );
      ...
      htmlContent = htmlContent
        .replace("</head>", `<link rel="stylesheet" href="${styleUri}"></head>`)
        .replace('src="reset-icon.png"', `src="${resetIconPath}"`)
        .replace('src="send-icon.png"', `src="${sendIconPath}"`)
        .replace(
